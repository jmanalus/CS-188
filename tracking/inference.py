...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        until = util.Counter()
        for p in self.legalPositions:
            if p not in self.beliefs: 
                continue
            cur_prob, current,  = self.beliefs[p], self.setGhostPosition(gameState, p)
            dist = self.getPositionDistribution(current)
            for future, fut_prob in dist.items():
                until[future] += fut_prob * cur_prob
        self.beliefs = until
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.particles:
            weights[p] += emissionModel[util.manhattanDistance(pacmanPosition, p)]
        weights.normalize()
        if weights.totalCount() == 0:
            self.initializeUniformly(gameState)
            return
        for x in range(self.numParticles):
            self.particles[x] = util.sample(weights)
    ...
    def elapseTime(self, gameState):
        ...
        for i in self.particles:
          dist = self.getPositionDistribution(self.setGhostPosition(gameState, i))
          s = util.sample(dist)
          n.append(s)
        self.particles = n
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [product[int(1.0*i/self.numParticles*len(product))] for i in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
        ...
        for i in self.particles:
            ...
            for g in range(self.numGhosts):
                if noisyDistances[g] == None:
                    lst[g] = self.getJailPosition(g)
                    i = tuple(lst) 
                else:
                    tdist = util.manhattanDistance(i[g],pacmanPosition)
                    weight *= emissionModels[g][tdist]
            dist[i] += weight
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for g in range(self.numGhosts):
                newPos = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), g, self.ghostAgents[g])
                newParticle[g] = util.sampleFromCounter(newPos)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            dist[particle] += 1.0 / self.numParticles
        return dist
