...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        dist, bestMove = 100, None
        for f in livingGhostPositionDistributions:
          ghost = max(f, key=f.get)
          for move in legal:
            pacman = Actions.getSuccessor(pacmanPosition, move)
            ghostDist = self.distancer.getDistance(ghost, pacman)
            if ghostDist < dist:
              dist, bestMove = ghostDist, move
        return bestMove
